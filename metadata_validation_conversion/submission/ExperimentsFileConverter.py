...
class ExperimentFileConverter:
    def __init__(self, json_to_convert, room_id, private):
    ...
    def generate_experiment_xml(self):
        ...
        for record in self.json_to_convert['experiment_ena']:
            ...
            print(alias)
            print(json.dumps(faang_experiment))
    ...
    def generate_submission_xml(self, private_study_submission=False):
        """
        ...
        for record in self.json_to_convert['submission']:
            if private_study_submission:
                submission_elt = etree.SubElement(
                    submission_set, 'SUBMISSION',
                    alias=f"{record['alias']}_private_study_submission")
                filename = f"{self.room_id}_private_study_submission.xml"
            else:
            ...
            # Add HOLD HoldUnitData tag when have private submission
