...
def get_species_for_project(project):
    ...
    r = requests.get(f'http://backend-svc:8000/data/organism/_search/?size=10000&filters={project_filter}')
    data = json.loads(r.content)['hits']['hits']
...
def update_ontology_summary():
    r = requests.get(f'http://backend-svc:8000/data/ontologies/_search/?size=10000')
    if r.status_code == 200:
        ontologies = json.loads(r.content)['hits']['hits']
        ...
        df = df.loc[df['projects'].isin(valid_projects)]
        ...
        url = f"http://backend-svc:8000/data/summary_ontologies/_search/?size=10"
        res = requests.get(url)
        res_data = json.loads(res.content)['hits']['hits']
        validated_counts = {}
